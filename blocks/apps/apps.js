...
export default async function decorate(block) {
  ...
  let heroImgBefore = false;
  [...block.children].forEach((tab, idx) => {
    if (tab.querySelector('picture')) {
      tab.className = 'hero-img';
      heroImgBefore = true;
      return;
    }

    const tabIndex = heroImgBefore ? idx - 1 : idx;
    ...
    const fragmentLink = tabButton.querySelector('a[href*="/fragments/"]:only-child');
    if (fragmentLink) {
      tabPanel.append(fragmentLink.cloneNode(true));
      fragmentLink.parentElement.textContent = fragmentLink.textContent;
    }
